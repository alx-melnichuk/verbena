...
mod tests {
    ...
    #[actix_web::test]
    async fn test_get_ws_chat_ews_join_not_auth_err_not_param() {
        // Create a test server without listening on a port.
        let mut srv = actix_test::start(move || {
            let (cfg_c, data_c, _token) = get_cfg_data(0);
            App::new().service(get_ws_chat).configure(configure_chat_message(cfg_c, data_c))
    ...
    #[actix_web::test]
    async fn test_get_ws_chat_ews_join_not_auth_err_stream_not_active() {
        // Create a test server without listening on a port.
        let mut srv = actix_test::start(move || {
            let (cfg_c, data_c, _token) = get_cfg_data(0);
            App::new().service(get_ws_chat).configure(configure_chat_message(cfg_c, data_c))
    ...
    #[actix_web::test]
    async fn test_get_ws_chat_ews_join_auth_err_not_param() {
        ...
        let mut framed = srv.ws_at(URL_WS).await.unwrap();
        #[rustfmt::skip]
        let msg_text = MessageText(format!("{{ \"join\": {}, \"access\": \"{}\" }}", i32::default(), token1).into());
        framed.send(msg_text).await.unwrap(); // Send a message to a websocket.
        let item = framed.next().await.unwrap().unwrap(); // Receive a message from a websocket.
        #[rustfmt::skip]
        assert_eq!(item,FrameText(Bytes::from(format!("{{\"err\":\"'{}' {}\"}}", "join", session::PARAMETER_NOT_DEFINED))));
    ...
    #[actix_web::test]
    async fn test_get_ws_chat_ews_join_auth_err_stream_not_active() {
        let (profile_vec, _session_vec) = get_profiles(2);
        let user_id1 = profile_vec.get(0).unwrap().user_id;
        let token1 = get_token(config_jwt::get_test_config(), user_id1);
        // Create a test server without listening on a port.
    ...
    #[actix_web::test]
    async fn test_get_ws_chat_ews_join_auth_err_token() {
        let (profile_vec, _session_vec) = get_profiles(2);
        let user_id1 = profile_vec.get(0).unwrap().user_id;
    ...
    #[actix_web::test]
    async fn test_get_ws_chat_ews_join_auth_err_dual_connect() {
        let (profile_vec, _session_vec) = get_profiles(2);
        let user_id1 = profile_vec.get(0).unwrap().user_id;
        let member1 = profile_vec.get(0).unwrap().nickname.clone();
        let token1 = get_token(config_jwt::get_test_config(), user_id1);
    ...
    #[actix_web::test]
    async fn test_get_ws_chat_ews_join_auth_err_stream_id_wrong() {
        let (profile_vec, _session_vec) = get_profiles(2);
        let user_id1 = profile_vec.get(0).unwrap().user_id;
        let token1 = get_token(config_jwt::get_test_config(), user_id1);
    ...
    #[actix_web::test]
    async fn test_get_ws_chat_ews_join_auth_err_session_not_found() {
    ...
    #[actix_web::test]
    async fn test_get_ws_chat_ews_join_auth_ok_owner() {
        let (profile_vec, _session_vec) = get_profiles(2);
        let user_id1 = profile_vec.get(0).unwrap().user_id;
        let member1 = profile_vec.get(0).unwrap().nickname.clone();
        let token1 = get_token(config_jwt::get_test_config(), user_id1);
    ...
    #[actix_web::test]
    async fn test_get_ws_chat_ews_join_auth_ok_not_owner() {
        let (profile_vec, _session_vec) = get_profiles(2);
    ...
    #[actix_web::test]
    async fn test_get_ws_chat_ews_join_auth_ok_blocked() {
        let (profile_vec, _session_vec) = get_profiles(4);
