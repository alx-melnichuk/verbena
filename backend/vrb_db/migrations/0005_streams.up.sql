-- Adding entities: "streams", "stream_tags", "link_stream_tags_to_streams".
...
/* Creating a trigger to fill in the "created_at" and "updated_at" fields. */
SELECT manage_updated_at('streams');

/* Create indexes for the "streams" table. */
...
/* Creating the "stream_tags" table. */
...
/* Create indexes for the "stream_tags" table. */
...
/* Creating the "link_stream_tags_to_streams" table. */
...
/* Create indexes for the "link_stream_tags_to_streams" table. */
...
BEGIN
  ...
  WHILE idx1 <= len1 LOOP
    ...
    SELECT s.id, s.user_id, s.title, s.descript, s.logo, s.starttime, s.live, s."state",
      s."started", s."paused", s."stopped", s.source, s.created_at, s.updated_at, tags, old_logo
    FROM streams s
    WHERE s.id = mark_id;
