-- Remove chats for streams of 5 test users.
...
BEGIN
  ...
  WHILE idx > 0 LOOP
    ...
    WITH deleted AS (
      ...
      RETURNING 1
    ) SELECT count(*) FROM deleted INTO cnt;
    raise notice 'For the "%", % records in the chat_messages have been deleted.', nick, cnt;
