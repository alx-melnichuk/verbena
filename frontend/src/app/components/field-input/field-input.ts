...
@Component({
    ...
    providers: [
        { provide: NG_VALUE_ACCESSOR, useExisting: forwardRef(() => FieldInput), multi: true },
...
export class FieldInput implements OnChanges, ControlValueAccessor, Validator {
    ...
    @Input()
    public type: string | null | undefined = "text";
    ...
    ngOnChanges(changes: SimpleChanges): void {
        if (!!changes["isRequired"] || !!changes["minLen"] || !!changes["maxLen"] || !!changes["pattern"] || !!changes["type"]) {
            this.prepareFormGroup(this.isRequired || null, this.maxLen || null, this.minLen || null, this.pattern || null
                , this.type || null);
        }
