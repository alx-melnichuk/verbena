...
@Component({
    selector: "app-field-color-theme",
    exportAs: "appFieldColorTheme",
    standalone: true,
    imports: [CommonModule, ReactiveFormsModule, MatFormFieldModule,
        MatInputModule, MatSelectModule, TranslatePipe],
    templateUrl: "./field-color-theme.html",
    styleUrl: "./field-color-theme.scss",
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    providers: [
        { provide: NG_VALUE_ACCESSOR, useExisting: forwardRef(() => FieldColorTheme), multi: true },
...
export class FieldColorTheme implements OnChanges, ControlValueAccessor, Validator {
    ...
    @Input()
    public kind: string = COLOR_THEME;
    @Input()
    ...
    public getErrorMsg(errors: ValidationErrors | null): string {
        ...
        return !!key ? `417.field-${this.kind || COLOR_THEME}:${key}` : "";
    }
    public getErrorObj(errors: ValidationErrors | null, value: string | null | undefined): ValidationErrors {
        return { ...errors, ...{ [`field-${this.kind || COLOR_THEME}`]: value } };
    }
