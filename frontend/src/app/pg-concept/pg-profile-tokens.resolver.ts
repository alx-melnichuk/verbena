...
import { TokenUserResponseDto } from '../lib-profile/profile-api.interface';

export const pgProfileTokensResolver: ResolveFn<TokenUserResponseDto | HttpErrorResponse | undefined> =
    (_route: ActivatedRouteSnapshot, _state: RouterStateSnapshot) => {
        const router = inject(Router);
        const profileService: ProfileService = inject(ProfileService);
        const profileTokensDto: TokenUserResponseDto | null = profileService.profileTokensDto;
