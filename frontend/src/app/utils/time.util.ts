export class TimeUtil {
    ...
    public static addTime(
        ...
        const timeValue = TimeUtil.parseTime(value); // { hours: number, minutes: number, seconds: number } | null
        if (!!timeValue) {
            const date1 = new Date(Date.now());
            date1.setHours(timeValue.hours + addHours, timeValue.minutes + addMinutes, timeValue.seconds + addSeconds, 0);
            hours = date1.getHours();
            minutes = date1.getMinutes();
            seconds = date1.getSeconds();
        }
        return (!!timeValue ? { hours, minutes, seconds } : null);
    }
}